    ...
    <section className="space-y-3" aria-labelledby="privacy-heading">
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        {state.capabilities.export && (
          <Button variant="outline" size="sm" disabled={loading} onClick={() => run("export", { include_photos: false })}>
            <Download className="mr-1 h-3.5 w-3.5" /> Privacy-export
          </Button>
        )}
        {state.capabilities.archive && (
          <Button variant="outline" size="sm" disabled={loading} onClick={() => run(archived ? "restore" : "archive")}>
        ...
        <div className="space-y-2 rounded-lg border border-border p-3">
          ...
          {state.legal_hold ? (
            <div className="space-y-2">
              ...
              <Button variant="outline" size="sm" disabled={loading} onClick={() => run("release_hold")}>Opheffen</Button>
            </div>
          ) : (
            <div className="space-y-2">
              ...
              <Button variant="outline" size="sm" disabled={loading || reason.trim().length < 3} onClick={() => run("hold", { reason }).then((ok) => ok && setReason(""))}>Instellen</Button>
            </div>
          )}
        </div>
      )}
      ...
      {(state.capabilities.pseudonymize || state.capabilities.delete) && (
        <div className="flex flex-wrap gap-2 border-t border-border pt-3">
          {state.capabilities.pseudonymize && (
            <Button variant="outline" size="sm" disabled={loading || !!state.legal_hold} onClick={() => { setConfirmation(""); setDangerous("pseudonymize"); }}>
              <UserRoundX className="mr-1 h-3.5 w-3.5" /> Pseudonimiseren
            </Button>
          )}
          {state.capabilities.delete && (
            <Button variant="destructive" size="sm" disabled={loading || state.preflight.blocked} onClick={() => { setConfirmation(""); setDangerous("delete"); }}>
