          ...
          <div className="space-y-2">
            {loading ? <p className="text-sm text-muted-foreground text-center py-6">Leden laden...</p> : enriched.length === 0 ? <p className="text-sm text-muted-foreground text-center py-6">Nog geen leden. Voeg handmatig een lid toe{features.white_label_signup && features.member_portal ? " of deel de aanmeldpagina" : ""}.</p> : enriched.map((member: any) => (
              <div key={member.id} className="p-4 rounded-xl bg-secondary/30 flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                ...
                <div className="flex flex-col sm:flex-row gap-2 flex-wrap">{member.status === "payment_issue" && <Button variant="gradient" size="sm" onClick={async () => { setBusyId(member.id); const { data, error } = await supabase.functions.invoke("create-payment", { body: { amount: Number(member.plan?.price || 0), payment_type: "membership", membership_id: member.id, customer_id: member.customer_id, method: "ideal", redirect_url: window.location.href } }); setBusyId(null); if (error) { toast.error("Kon betaallink niet aanmaken"); return; } if ((data as any)?.demo) { toast.success("Demo: betaallink gesimuleerd"); return; } if ((data as any)?.checkoutUrl) { navigator.clipboard.writeText((data as any).checkoutUrl).then(() => toast.success("Nieuwe betaallink gekopieerd")); } }} disabled={busyId === member.id}><RefreshCw className="w-3.5 h-3.5" /> Probeer betaling opnieuw</Button>}{features.pause_allowed && <Button variant="outline" size="sm" onClick={() => updateMember(member.id, { status: member.status === "paused" ? "active" : "paused", paused_at: member.status === "paused" ? null : new Date().toISOString() }, member.status === "paused" ? "Lid heractiveerd" : "Lid gepauzeerd")} disabled={busyId === member.id}>{member.status === "paused" ? <Play className="w-3.5 h-3.5" /> : <Pause className="w-3.5 h-3.5" />}{member.status === "paused" ? "Heractiveer" : "Pauzeer"}</Button>}{features.credits_system && <Button variant="outline" size="sm" onClick={() => updateMember(member.id, { credits_available: Number(member.credits_available || 0) + 1 }, "Credit toegevoegd")} disabled={busyId === member.id}><Gift className="w-3.5 h-3.5" /> Credit</Button>}{features.self_cancel && <Button variant="outline" size="sm" onClick={() => { const reason = window.prompt("Reden van opzegging (optioneel):", "") || null; updateMember(member.id, { status: "cancelled", cancel_at_period_end: true, cancelled_at: new Date().toISOString(), metadata: { ...(member.metadata || {}), cancellation_reason: reason } }, "Lid opgezegd per periode"); }} disabled={busyId === member.id}><XCircle className="w-3.5 h-3.5" /> Stop</Button>}</div>
              </div>
            ))}
          </div>
        </section>}
