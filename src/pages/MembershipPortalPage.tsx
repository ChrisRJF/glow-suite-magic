...
function callPublicMemberships<T>(body: Record<string, unknown>) {
  return supabase.functions.invoke("public-memberships", { body }).then(({ data, error }) => {
    if (error) throw new Error(error.message || "Memberships zijn niet bereikbaar");
...
export default function MembershipPortalPage() {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      setData(await callPublicMemberships<PortalData>({ action: "get_memberships", slug: salonSlug }));
    } catch (err: any) {
      setError(err.message || "Membershippagina kon niet worden geladen.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const join = async () => {
    ...
    try {
      const redirectUrl = `${window.location.origin}/memberships/${salonSlug}`;
  ...
  if (error || !data) return <div className="min-h-screen bg-background flex items-center justify-center p-4"><div className="glass-card p-6 max-w-sm text-center"><p className="text-sm font-semibold">Memberships niet beschikbaar</p><p className="text-xs text-muted-foreground mt-1">{error || "Deze pagina is nog niet actief."}</p><Button variant="outline" size="sm" className="mt-4" onClick={load}>Opnieuw proberen</Button></div></div>;
  if (confirmed) return <main className={cn("min-h-screen bg-background p-4", isEmbed && "min-h-0")}><div className="mx-auto max-w-lg glass-card p-6 text-center"><CheckCircle2 className="w-10 h-10 text-success mx-auto mb-3" /><h1 className="text-xl font-semibold">Aanmelding ontvangen</h1><p className="text-sm text-muted-foreground mt-2">Bedankt. Je membership bij {data.salon.name} wordt automatisch geactiveerd zodra de betaling is bevestigd.</p></div></main>;
