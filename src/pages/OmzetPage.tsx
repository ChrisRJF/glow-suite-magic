        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {predictions.map((p) => (
            <div key={p.label} className="stat-card">
              ...
              <p className="text-2xl font-bold mt-1">{p.isPercent ? `+${p.value}%` : formatEuro(p.value)}</p>
              <div className="flex items-center gap-1 mt-1 text-xs text-success"><TrendingUp className="w-3 h-3" /><span>Stijgend</span></div>
            </div>
          ))}
        </div>
